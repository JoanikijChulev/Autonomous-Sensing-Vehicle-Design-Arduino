...
byte stopDist = 20;                               //Minimum distance from an object to stop in cm
...
void moveForwardMeters(int meters) {
  // Convert meters to delay time
  long delayTime = 0;
  if (meters == 1) {
    delayTime = 3500;
  } else if (meters == 3) {
  ...
  if (delayTime == 0) {
    Serial.println("Invalid distance. Available options: 1m, 3m, 5m");
    return;
  }
...
void moveBackwardMeters(int meters) {
  // Convert meters to delay time
  long delayTime = 0;
  if (meters == 1) {
    delayTime = 3500;
...
void loop() {
      ...
      myservo.write(105);  // Rotate the servo to the current angle
